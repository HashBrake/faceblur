...
def test_weak_sightings_before_confirmation_are_covered():
    ...
    frames[5] = [det(100)]
    frames[6] = [det(105)]
    out, tracks = Tracker(min_track=2, max_gap=2, tail=2, tail_before=6).run(frames, weak)
    assert len(tracks) == 1
...
def test_the_head_tail_follows_the_motion_backwards_and_grows():
    ...
    out, _ = Tracker(min_track=2, max_gap=2, tail=0, tail_before=3, tail_grow=0.1).run(frames)
    three_back = out[3][0]
...
def test_the_foot_tail_follows_the_motion_forwards():
    ...
    frames[3] = [det(100)]
    frames[4] = [det(110)]
    out, _ = Tracker(min_track=2, max_gap=2, tail=2, tail_before=0).run(frames)
